...
class TestUserRoutes:
    ...
    #                            test profile route                            #
    # ------------------------------------------------------------------------ #
    async def test_get_my_profile(self, test_client, test_db):
    ...
    async def test_get_my_profile_no_auth(self, test_client, test_db):
    ...
    # ------------------------------------------------------------------------ #
    async def test_admin_can_get_all_users(self, test_client, test_db):
    ...
    async def test_admin_can_get_one_user(self, test_client, test_db):
    ...
    async def test_user_cant_get_all_users(self, test_client, test_db):
    ...
    async def test_user_cant_get_single_user(self, test_client, test_db):
        """Test we can't get all users if not admin."""
        for _ in range(3):
            await UserManager.register(
                self.get_test_user(hashed=False), session=test_db
    ...
    # ------------------------------------------------------------------------ #
    async def test_make_admin_as_admin(self, test_client, test_db):
    ...
    async def test_cant_make_admin_as_user(self, test_client, test_db):
        """Test we can upgrade an existing user to admin."""
        normal_user = self.get_test_user()
    ...
    #                            test ban user route                           #
    # ------------------------------------------------------------------------ #
    async def test_admin_can_ban_user(self, test_client, test_db):
    ...
    async def test_admin_cant_ban_self(self, test_client, test_db):
        """Ensure an admin can ban a user."""
        admin_user = {**self.get_test_user(), "role": RoleType.admin}
        admin_id = await test_db.execute(User.insert().values(**admin_user))
        token = AuthManager.encode_token({"id": admin_id})
    ...
    async def test_user_cant_ban(self, test_client, test_db):
    ...
    async def test_admin_cant_ban_missing_user(self, test_client, test_db):
    ...
    # ------------------------------------------------------------------------ #
    async def test_admin_can_unban_user(self, test_client, test_db):
        """Ensure an admin can ban a user."""
        normal_user = self.get_test_user()
        admin_user = {**self.get_test_user(), "role": RoleType.admin}
    ...
    async def test_admin_cant_uban_self(self, test_client, test_db):
    ...
    async def test_admin_cant_unban_missing_user(self, test_client, test_db):
        """Ensure an admin cant unban user that does not exist."""
        admin_user = {**self.get_test_user(), "role": RoleType.admin}
        admin_id = await test_db.execute(User.insert().values(**admin_user))
        token = AuthManager.encode_token({"id": admin_id})
    ...
    async def test_user_cant_unban(self, test_client, test_db):
    ...
    # ------------------------------------------------------------------------ #
    async def test_admin_can_delete_user(self, test_client, test_db):
    ...
    async def test_non_admin_cant_delete_user(self, test_client, test_db):
    ...
    async def test_delete_missing_user(self, test_client, test_db):
    ...
    # ------------------------------------------------------------------------ #
    async def test_user_can_change_own_password(self, test_client, test_db):
    ...
    async def test_user_cant_change_others_password(self, test_client, test_db):
    ...
    async def test_admin_can_change_others_password(self, test_client, test_db):
    ...
    # ------------------------------------------------------------------------ #
    async def test_user_can_change_own_details(self, test_client, test_db):
    ...
    async def test_user_cant_change_others_details(self, test_client, test_db):
        """Ensure a user cant change other user password."""
        normal_user = self.get_test_user()
        normal_user2 = self.get_test_user()
        user_id = await test_db.execute(User.insert().values(**normal_user))
    ...
    async def test_admin_can_change_others_details(self, test_client, test_db):
        """Ensure an admin user can change any user password."""
        normal_user = self.get_test_user()
        admin_user = {**self.get_test_user(), "role": RoleType.admin}
        user_id = await test_db.execute(User.insert().values(**normal_user))
